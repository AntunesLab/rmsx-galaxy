...
The first dependency scaffold lives in `packaging/flipbook-galaxy/`. It builds a container from the upstream RMSX source repository and preinstalls the Python/R runtime stack. The active wrapper references a registry-qualified container tag, `ghcr.io/antuneslab/flipbook-galaxy:0.2.3-galaxy0`. Collaborators can build that tag locally with `scripts/build_container.sh` before it is published to GHCR.

For purely local development, you may temporarily retag or override the container as `flipbook-galaxy:0.2.3-galaxy0`, but the shareable wrapper should keep the registry-qualified tag in `tools/flipbook/macros.xml`.
...
The Galaxy tool form includes an input-source selector. "Use datasets from history" accepts uploaded topology/structure and DCD/XTC trajectory datasets, preserves their Galaxy extensions with job-local symlinks, and validates them with `rmsx_preflight.py` before RMSX runs. "Load example data: 1UBQ plus mon_sys" uses `tools/flipbook/test-data/1UBQ.pdb` and `tools/flipbook/test-data/mon_sys.xtc` directly from the wrapper directory. The example branch defaults to chain/segment ID `7` and nine trajectory slices so a fresh Galaxy session can produce the interactive reports without a manual upload step.
...
The runtime image in `packaging/flipbook-galaxy/` is built locally with the same tag that the Galaxy wrapper declares:
...
```bash
docker run --rm ghcr.io/antuneslab/flipbook-galaxy:0.2.3-galaxy0 rmsx --help
```
